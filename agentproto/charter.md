...
The dialog context is defined so that existing application-layer agent communication protocols (e.g., the Model Context Protocol (MCP) and the Agent2Agent (A2A) protocol) can carry it using their own extension mechanisms. Changes to those protocols themselves are the responsibility of their respective maintainers and are, thus, out of scope of agentproto.
